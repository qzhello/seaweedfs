...
export function TasksPanel() {
  ...
  return (
    <div className="space-y-6">
      {/* Toolbar (no page H1 — the tab strip in /activity provides title context) */}
                ...
                <tr>
                  ...
                  <td className="text-right space-x-1">
                    ...
                    {task.status === "running" && (
                      <button className="btn btn-danger" disabled={busy===task.id}
                        onClick={async () => {
                          const ok = await confirmDlg.warning({
                            title: "Force-stop this task?",
                            body: "The running SeaweedFS command will be interrupted and may leave the volume in an intermediate state.",
                            confirmLabel: "Force stop",
                          });
                          if (ok) act(() => api.stopTask(task.id), task.id);
                        }}>
                        <Square size={14}/> Stop
                      </button>
                    )}
