...
export function AIConfigPanel() {
  ...
  return (
    <div className="space-y-6">
      {/* Toolbar — page title lives in the /admin tab strip */}
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <p className="text-sm text-muted">
            {t("Works with OpenAI, Anthropic Claude, DeepSeek, Ollama, and any OpenAI-compatible gateway.")}
            {t("API keys are encrypted at rest in PostgreSQL with AES-GCM.")}
          </p>
          {resp?.current && (
            <p className="text-xs text-muted mt-1">
              {t("Active:")} <span className="font-mono text-accent">{resp.current}</span>
            </p>
          )}
        </div>
        <div className="flex items-center gap-2">
          <Link href="/ai/tools" className="btn inline-flex items-center gap-1.5">
            <ShieldCheck size={14}/> {t("Tool authorization")}
          </Link>
          ...
          <ProviderCard key={row.id} row={row} tpl={tplByKind[row.kind]}
            ...
            onDelete={async () => {
              if (!(await confirmDlg.danger({
                title: t('Delete provider "{name}"?').replace("{name}", row.name),
                body: t("Tools and skills bound to this provider will fall back to the default. You can recreate it later."),
        ...
        {(resp?.items?.length ?? 0) === 0 && (
          <div className="col-span-full">
            <EmptyState icon={Sparkles}
              title={t("No AI providers configured")}
              hint={t('Click "Add provider" above to wire OpenAI, Anthropic, or another vendor.')}/>
          </div>
        )}
      </section>
  ...
  return (
    <div className={`card p-5 space-y-3 ${row.is_default ? "border-accent/40 bg-accent/5" : ""}`}>
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            {row.is_default && (
              <span className="text-accent flex items-center gap-0.5 text-xs">
                <Star size={12} fill="currentColor"/> {t("default")}
              </span>
            )}
            {!row.enabled && <span className="text-xs text-muted">{t("(disabled)")}</span>}
          </div>
      ...
      <div className={`text-xs ${testTone} flex items-center gap-1.5`}>
        {row.last_test_at == null && <span>{t("not tested")}</span>}
        {row.last_test_ok && <CheckCircle2 size={12}/>}
        {row.last_test_ok === false && <XCircle size={12}/>}
        {row.last_test_at && (
          <>
            <span>{row.last_test_ok ? t("OK") : t("FAIL")}</span>
      ...
      <div className="flex items-center gap-2 text-xs pt-2 border-t border-border">
        ...
        <button onClick={onEdit} className="btn text-xs">{t("Edit")}</button>
        {!row.is_default && (
          <button onClick={onSetDefault} className="btn text-xs">{t("Set as default")}</button>
        )}
...
function SecretBadge({ row }: { row: ProviderRow }) {
  ...
  if (row.has_encrypted_secret) {
    return (
      <span title={t("API key stored encrypted")}
        className="px-2 py-0.5 rounded-md border border-success/40 text-success text-[11px] flex items-center gap-1">
        <KeyRound size={10}/> {t("encrypted")}
      </span>
    );
  }
  if (row.secret_ref) {
    return (
      <span title={t("Read from env var {name}").replace("{name}", row.secret_ref)}
        className="px-2 py-0.5 rounded-md border border-warning/40 text-warning text-[11px] flex items-center gap-1">
        <KeyRound size={10}/> {t("env")}
      </span>
    );
  }
  if (row.kind === "rule") return <span className="text-[11px] text-muted">{t("no credentials")}</span>;
  return (
...
function ProviderEditor({
  ...
  const submit = async () => {
    ...
    catch (e) { setErr(e instanceof Error ? e.message : t("save failed")); }
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 overflow-auto">
      <div className="bg-panel border border-border rounded-xl w-full max-w-2xl shadow-2xl">
        <div className="px-5 py-4 border-b border-border flex items-center justify-between">
          <h2 className="text-lg font-medium">{isEdit ? t("Edit provider") : t("New provider")}</h2>
          <button onClick={onClose} aria-label={t("Close")} className="text-muted hover:text-text"><X size={16}/></button>
        </div>
        <div className="p-5 space-y-4">
          <div>
            <div className="text-xs text-muted mb-2">{t("Vendor")}</div>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            {draft.kind !== "rule" && (
              <Field label={t("Base URL")} full>
                <input className="input font-mono" value={(config.base_url as string) ?? ""}
                  placeholder={draft.kind === "anthropic" ? t("https://api.anthropic.com (blank = official endpoint)") : ""}
                  onChange={e => updateConfig("base_url", e.target.value)}/>
              </Field>
            )}
            {draft.kind !== "rule" && (
              <Field label={tpl?.key_hint ? t("API key (e.g. {hint})").replace("{hint}", tpl.key_hint) : t("API key")} full>
                <input className="input font-mono" type="password"
                  placeholder={isEdit && draft.has_encrypted_secret ? t("Stored — blank keeps it, fill to replace") : t("Paste API key")}
                  value={draft.api_key ?? ""}
                  onChange={e => onChange({ ...draft, api_key: e.target.value, clear_secret: false })}/>
                <div className="text-[11px] text-muted mt-1">
                  {t("Encrypted at rest with AES-GCM; never visible to the UI again. Or use an env var ↓")}
                </div>
              </Field>
            )}
            {draft.kind !== "rule" && (
              <Field label={t("Or: env var name")} full>
            ...
            {isEdit && draft.has_encrypted_secret && (
              <label className="text-xs text-warning col-span-2 flex items-center gap-2">
                ...
                {t("Clear the stored encrypted key, fall back to env var")}
              </label>
            )}
