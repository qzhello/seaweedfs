...
// the Costs dashboard). Local-disk pseudo-backends are pre-seeded by
// migration 036.
...
const KIND_TONE: Record<BackendPricing["kind"], string> = {
  hot:     "border-rose-400/40 text-rose-300 bg-rose-400/5",
  warm:    "border-amber-400/40 text-amber-300 bg-amber-400/5",
  cold:    "border-sky-400/40 text-sky-300 bg-sky-400/5",
      ...
      <header className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-base font-semibold tracking-tight inline-flex items-center gap-2">
            <DollarSign size={20}/> {t("Backend pricing")}
          </h1>
          <p className="text-xs text-muted mt-1 max-w-2xl">
            {t("$/TB/month per storage backend. Exactly one row must be the hot reference — its price is the basis for 'savings vs all-hot-3x' on the Costs dashboard.")}
          </p>
        </div>
                    ...
                    <div className="flex gap-1 justify-end">
                      <Can cap="cost.write">
                        ...
                        <button
                          ...
                          title={t("Delete")}
                          onClick={async () => {
                            if (!confirm(t('Delete pricing for "{name}"?').replace("{name}", p.name))) return;
                            try {
