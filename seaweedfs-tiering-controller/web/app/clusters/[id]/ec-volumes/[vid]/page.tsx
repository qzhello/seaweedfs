...
import { Layers, AlertTriangle, Server, Wrench } from "lucide-react";
import {
...
export default function ECVolumeDetailPage() {
  ...
  const { t } = useT();
  const { data, error, mutate } = useECVolumeDetail(id, vid);
    ...
    <div className="space-y-5">
      ...
      {data ? (
        <>
          ...
          {/* Stats row — rendered as a div to keep the single persistent header above */}
          <div className="text-xs text-muted mt-0.5">
            ...
            <HealthBadge tone={data.healthy ? "ok" : "err"}>
              {data.shards_present}/{data.total_shards} {t("shards")}
            </HealthBadge>
          </div>
          ...
          <section className="card p-4 space-y-2">
            ...
            <div className="overflow-x-auto">
              <div className="grid gap-1" style={{ gridTemplateColumns: `repeat(${data.total_shards}, 32px)` }}>
                {Array.from({ length: data.total_shards }).map((_, idx) => {
