...
import { Loader2, Layers, Scale, ArrowLeft } from "lucide-react";
...
export default function ClusterCollectionDetailPage() {
  ...
  const { t } = useT();
  const { data, isLoading, error } = useCollectionDetail(id, decoded);
  ...
  if (isLoading || !data) {
    return (
      <div className="p-6 text-sm text-muted inline-flex items-center gap-2">
        <Loader2 size={14} className="animate-spin"/> {t("Loading collection…")}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <header className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          <Link href="/collections" className="text-xs text-muted inline-flex items-center gap-1 hover:text-text">
            <ArrowLeft size={12}/> {t("Back to all collections")}
          </Link>
          <h1 className="text-base font-semibold tracking-tight inline-flex items-center gap-2 mt-1">
            <Layers size={18}/> {displayName}
          </h1>
