...
import {
  DollarSign, Sparkles, Snowflake, Flame, TrendingDown, RefreshCw, Camera,
      ...
      <header className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-base font-semibold tracking-tight inline-flex items-center gap-2">
            <DollarSign size={20}/> {t("Costs")}
          </h1>
          <p className="text-xs text-muted mt-1 max-w-2xl">
            {t("Current monthly storage spend per backend and per collection. Reasoned against the counterfactual: 'what if every byte were on hot with 3 replicas?'")}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Link href="/pricing" className="btn text-xs inline-flex items-center gap-1.5">
            <DollarSign size={12}/> {t("Manage pricing")}
          </Link>
      ...
      {isLoading && !data ? (
        ...
        <>
          ...
          {data.unpriced_bytes > 0 && (
            <div className="card p-3 border-amber-400/40 bg-amber-400/5 text-xs text-amber-200 inline-flex items-start gap-2">
              <AlertTriangle size={14} className="shrink-0 mt-0.5"/>
              <div>
                {t("{n} of storage is on backends with no pricing configured.").replace("{n}", fmtBytes(data.unpriced_bytes))}{" "}
                <Link href="/pricing" className="underline text-text">{t("Configure pricing")}</Link>{" "}
                  ...
                  return (
                    <tr key={b.name}>
                      ...
                      <td className="text-right font-mono text-xs">
                        {b.has_pricing ? `${b.currency} ${b.price_per_tb_month.toFixed(2)}` : <span className="text-amber-300">—</span>}
                      </td>
          ...
          {data.backends.length === 0 && (
            <EmptyState icon={DollarSign} title={t("No volumes to price yet")} hint={t("Once the cluster has volumes, this dashboard fills in. Make sure pricing is configured in /pricing.")}/>
          )}
        </>
      )}
    </div>
            ...
            return (
              <div key={m} className="flex flex-col items-center gap-0.5 w-12">
                <div className="flex-1 flex items-end gap-0.5 w-full">
                  <div className="bg-rose-500/40 w-1/2 rounded-t" title={t("counterfactual")}
        ...
        <tbody>
          {recs.map((r, i) => (
            <tr key={i}>
              ...
              <td className="text-right font-mono text-sm text-emerald-300">{r.currency} {r.monthly_saving.toFixed(2)}</td>
  ...
  return (
    <section className="card overflow-hidden border-accent/40 bg-accent/[0.03]">
      <header className="px-3 py-2 border-b border-border inline-flex items-center gap-2 text-xs font-semibold">
        <Sparkles size={12} className="text-amber-300"/> {t("AI migration proposals")}
      </header>
      {summary && (
